...
\smallskip
Por ello durante el reporte se hará incapié en  las ventajas del uso de ciertos algoritmos respecto a otras alternativas posibles.
...
Todos estos objetos estan contenidos en un arreglo de tipo $orden$.
...
\begin{enumerate}
    \item Cache friendly, ya que no genera subarreglos, librandonos de ocupar memoria adicional y dando una complejidad espacial de \texttt{O(n)}.
    
    \item Debido al diseño este algoritmo y nuestro caso particular de lista desordenada podemos decir que la complejidad temporal para este casó será \texttt{O(nlog(n))}.
    \smallskip
...
Para la implementación de este requerimiento de la situación problema, simplemente se iteró sobre el arreglo ya ordenado hasta el indice 9 para mostrarlos en pantalla.
...
\end{figure}
 ...
 Cuidando que para fecha de inicio de busqueda se retorne el valor exacto o el más aproximado para arriba, en cambio para el de fin de búsqueda el más cercano para abajo.
 ...
 Este método de busqueda tiene una complejidad temporal \texttt{O(log(n))}, y espacial \texttt{O(1)}.
 \medskip
 ...
 Así como reducir la complejidad temporal de la impresión de los datos a O(k), siendo k el número de elementos a imprimir.
  ...
  \begin{minipage}{0.55\textwidth}
    ...
    \caption{Busqueda binaria: \cite{onlinegdb_situacion_problema}}
